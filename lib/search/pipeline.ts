...
export function toRow(
  ...
  return {
    ...
    cleaningFee: breakdown?.cleaningFee ?? null,
    serviceFee: breakdown?.serviceFee ?? null,
    taxes: breakdown?.taxes ?? null,
    discountTotal: breakdown?.discountTotal ?? null,
    pricePerPerson: breakdown?.pricePerPerson ?? null,
    isAvailable: true,
    // A busca já entrega o total real quando a origem exibe o preço regulado;
    // fixar "search" aqui descartaria essa informação e mandaria o usuário
    // recotar o que já está confirmado.
    priceSource: breakdown?.source ?? "search",
    cleaningRatio: breakdown ? cleaningRatio(breakdown) : null,
    priceHonesty: breakdown ? priceHonesty(breakdown) : null,
